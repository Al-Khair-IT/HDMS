...
**Recommended Architecture:** Service-Oriented Modular Monolith

* Backend and frontend communicate via RESTful and WebSocket APIs.
...
This ensures clean separation of logic while keeping the MVP lightweight and easy to deploy in one container or VM.
...
# **Backend Architecture — Help Desk Management System**
...
* **File Storage:** Cloud / Local FileSystem (for attachments)
...
`├─ .env`  
`│`  
`├─ core/                            # Global project config (settings, middleware)`  
`│  ├─ __init__.py`  
`│  ├─ asgi.py                       # Entry for async (channels + socket)`  
`│  ├─ wsgi.py`  
`│  ├─ settings/`  
`│  │  ├─ __init__.py`  
`│  │  ├─ base.py`  
...
`│  │  └─ logging.py`  
`│  ├─ urls.py`  
`│  ├─ routers.py                    # Include all app routes here`  
`│  ├─ schema.py                     # Global OpenAPI schema (Ninja)`  
`│  └─ middleware/`  
`│     ├─ auth_middleware.py`  
`│     └─ request_logger.py`  
`│`  
`├─ apps/`  
`│  ├─ users/`  
`│  │  ├─ models.py                  # Integrated with SMS employee table`  
`│  │  ├─ api.py                     # Ninja endpoints for profile & roles`  
`│  │  ├─ services.py                # Role-based logic helpers`  
...
`│  │  ├─ selectors.py               # Read-only optimized queries`  
`│  │  ├─ signals.py`  
`│  │  ├─ utils.py`  
`│  │  ├─ tasks.py                   # Celery scheduled jobs (reminders)`  
`│  │  ├─ admin.py`  
`│  │  └─ tests/`  
`│  │     ├─ test_ticket_flow.py`  
...
`│  │`  
`│  ├─ chat/`  
`│  │  ├─ consumers.py               # WebSocket consumers for ticket chat`  
`│  │  ├─ events.py                  # Event mappings for WebSocket events`  
`│  │  ├─ routing.py`  
`│  │  ├─ serializers.py`  
`│  │  ├─ utils.py`  
`│  │  └─ services.py`  
...
`│  │  ├─ api.py`  
`│  │  ├─ services.py`  
`│  │  ├─ tasks.py                   # Celery jobs for scheduled notifications`  
`│  │  └─ selectors.py`  
`│  │`  
`│  ├─ departments/`  
`│  │  ├─ models.py`  
...
`│  ├─ audit/`  
`│  │  ├─ models.py`  
`│  │  ├─ middleware.py              # Auto-log create/update/delete (soft delete)`  
`│  │  ├─ services.py`  
`│  │  ├─ api.py`  
`│  │  └─ selectors.py`  
`│  │`  
`│  └─ integrations/`  
`│     ├─ sms_sync/`  
`│     │  ├─ client.py               # API connector to SMS system`  
`│     │  ├─ sync_tasks.py`  
`│     │  └─ utils.py`  
`│     ├─ email/`  
`│     │  ├─ templates/`  
...
Main project config — handles startup, routing, and environment segregation.

* `asgi.py`: entry point for async requests and sockets

* `settings/`: split by environment (base/dev/prod)

* `routers.py`: collects all app routers and mounts in the main Ninja API

* `middleware/`: custom middleware for audit or auth tracking

🟢 When to modify:  
 Add global middleware, update settings, or register a new app.

---

### **🧩 `apps/users/`**

Centralized user and role management (integrated with SMS).

* Mirrors `employee` data from SMS via scheduled sync.

* Handles JWT authentication and RBAC (Role-Based Access Control).

* Provides APIs for profile, permissions, and roles.

---

### **🧱 `apps/tickets/`**

**Core of the system** — handles ticket lifecycle, sub-tickets, comments, and priority logic.

| File / Folder | Purpose |
| ----- | ----- |
| `models/` | Ticket, SubTicket, Comment, Attachment, Audit models |
| `api/routes.py` | All endpoints (CRUD + filters + reports) |
| `services/` | Business logic (split, merge, reassign, postpone, restart) |
| `selectors.py` | Optimized read-only queries |
| `tasks.py` | Celery reminders (e.g., postponed ticket alerts every 3 days, auto-close reminders) |
| `signals.py` | Auto-logging on status changes |
| `admin.py` | Admin panel setup |
| `tests/` | Unit tests for ticket flow and notifications |

🟢 When to modify:  
 Whenever workflow or business logic changes.  
🛑 Avoid putting business logic inside views — always in `services`.

---

**💬 `apps/chat/`**

Handles **real-time chat** within each ticket.

* `consumers.py`: WebSocket consumer (connect, disconnect, receive message).

* `events.py`: Centralized mapping of WebSocket events (`message`, `join_room`, etc.).

* `services.py`: Save messages, handle joins/removes when the Moderator adds/removes users.

* `routing.py`: Channel routes.

🟢 Use Django Channels + Redis as channel layer.
...
Manages system and user-level notifications.

* For ticket status changes, assignments, postpones, restarts, etc.

* Uses Celery for scheduled notifications (reminders).

* Exposes `/notifications` API for frontend pull + socket push.  
* **Important:** When a ticket returns to an active state (Restarted or Resolved), the reminder task is immediately and reliably destroyed, preventing resource waste and phantom notifications.

---

### **🏢 `apps/departments/`**

Department registry and staff assignments.

* Tracks department capacity, active tickets, and available staff.

* Used by the Moderator to view department workload in real-time.

* Manages employee assignments to departments (by Admin).

---

### **🕵️ `apps/audit/`**

Centralized logging of all ticket lifecycle changes (status, reassignment, priority updates).

* Middleware hooks into model saves/deletes (soft deletes).

* Accessible via admin or API for history.

---

### **🔗 `apps/integrations/`**

Bridges between external systems (SMS, email, WhatsApp/Slack).  
 Each subfolder is a connector service.

| Submodule | Purpose |
| ----- | ----- |
| `sms_sync/` | Pull user data from Idara's SMS |
| `email/` | Send status or ticket-related emails |
| `slack_or_whatsapp/` | (optional future) message notifications |

---

### **🧰 `scripts/`**

For quick maintenance or migration tasks (e.g., seeding test data, resetting audit logs).
...
| Task | Where to Work |
| ----- | ----- |
| Add a new endpoint | `api/routes.py` in the respective app |
| Add a background task | `tasks.py` (Celery) |
| Modify business logic | `services/*.py` |
| Create an optimized query | `selectors.py` |
| Add WebSocket feature | `chat/consumers.py` |
| Add user sync or external integration | `integrations/sms_sync/` |
